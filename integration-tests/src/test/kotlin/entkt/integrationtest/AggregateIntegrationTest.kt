...
class AggregateIntegrationTest : PostgresTestBase() {
    ...
    private fun seed(c: EntClient) {
        c.orders.create { quantity = 2; price = 10.0; status = OrderStatus.PENDING; region = "us"; placedAt = t0 }.save(testViewerContext).getOrThrow()
        c.orders.create { quantity = 3; price = 20.0; status = OrderStatus.PENDING; region = "us"; placedAt = t0.plusSeconds(60) }.save(testViewerContext).getOrThrow()
        c.orders.create { quantity = 5; price = 30.0; status = OrderStatus.SHIPPED; region = null; placedAt = t0.plusSeconds(120) }.save(testViewerContext).getOrThrow()
        c.orders.create { quantity = 1; price = 5.0; status = OrderStatus.SHIPPED; region = "eu"; placedAt = t0.plusSeconds(180) }.save(testViewerContext).getOrThrow()
    }

    @Test
    fun `ungrouped scalar terminals are typed and correct`() {
        val c = client(); seed(c)
        assertEquals(4L, c.orders.query().rawCount(testViewerContext).getOrThrow())

        val sumQty: Long? = c.orders.query().rawSum(testViewerContext, Order.quantity).getOrThrow()   // integral → Long?
        assertEquals(11L, sumQty)
        val sumPrice: Double? = c.orders.query().rawSum(testViewerContext, Order.price).getOrThrow()  // floating → Double?
        assertEquals(65.0, sumPrice)
        val avgPrice: Double? = c.orders.query().rawAvg(testViewerContext, Order.price).getOrThrow()
        assertEquals(16.25, avgPrice)
        val maxQty: Int? = c.orders.query().rawMax(testViewerContext, Order.quantity).getOrThrow()    // min/max keep the column's type
        assertEquals(5, maxQty)
        assertEquals(t0, c.orders.query().rawMin(testViewerContext, Order.placedAt).getOrThrow())
        assertEquals(t0.plusSeconds(180), c.orders.query().rawMax(testViewerContext, Order.placedAt).getOrThrow())
    }
    ...
    fun `grouping by an enum decodes keys back to the enum`() {
        ...
        val byStatus: List<AggregateBucket<OrderStatus, Long>> =
            c.orders.query().rawCountBy(testViewerContext, Order.status).getOrThrow()
        assertEquals(
    ...
    fun `grouped sum and avg by an enum key`() {
        ...
        assertEquals(
            ...
            c.orders.query().rawSumBy(testViewerContext, Order.status, Order.quantity).getOrThrow().associate { it.key to it.value },
        )
        assertEquals(
            mapOf<OrderStatus, Double?>(OrderStatus.PENDING to 15.0, OrderStatus.SHIPPED to 17.5),
            c.orders.query().rawAvgBy(testViewerContext, Order.status, Order.price).getOrThrow().associate { it.key to it.value },
        )
    }
    ...
    fun `grouping by a nullable column folds NULLs into a null-key bucket`() {
        ...
        val byRegion: List<AggregateBucket<String?, Long>> =
            c.orders.query().rawCountBy(testViewerContext, Order.region).getOrThrow()
        assertEquals(
    ...
    fun `caller predicates narrow the aggregate`() {
        ...
        val shippedQty = c.orders.query { where(Order.status eq OrderStatus.SHIPPED) }
            .rawSum(testViewerContext, Order.quantity)
            .getOrThrow()
    ...
    fun `rawSum returns Success on the happy path`() {
        val c = client(); seed(c)
        val r = c.orders.query().rawSum(testViewerContext, Order.quantity)
    ...
    fun `empty set has count zero, null metrics, and no buckets`() {
        ...
        assertEquals(0L, c.orders.query().rawCount(testViewerContext).getOrThrow())
        assertNull(c.orders.query().rawSum(testViewerContext, Order.quantity).getOrThrow())
        assertNull(c.orders.query().rawAvg(testViewerContext, Order.price).getOrThrow())
        assertNull(c.orders.query().rawMax(testViewerContext, Order.placedAt).getOrThrow())
        assertTrue(c.orders.query().rawCountBy(testViewerContext, Order.status).getOrThrow().isEmpty())
    }
}
