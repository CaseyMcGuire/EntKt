...
class SymmetricLinkTableM2MPostgresIntegrationTest {
    ...
    fun `reads resolve the same relationship from both orientations`() {
        ...
        client.posts.create { title = "C (unlinked)" }.save(testViewerContext).getOrThrow()

        // Link A and B to the tag from the Post side.
        client.withTransaction { tx ->
            tx.posts.update(postA.id) { tags.add(tag.id) }.save(testViewerContext).orRollback()
            tx.posts.update(postB.id) { tags.add(tag.id) }.save(testViewerContext).orRollback()
        }.getOrThrow()

        // Read from the Tag side: the single tag reaches exactly {A, B}.
        val postsForTag = client.tags.query().queryPosts().all(testViewerContext).getOrThrow()
        ...
        val tagsReached = client.posts.query().queryTags().all(testViewerContext).getOrThrow()
    ...
    fun `cross-anchor re-add is an idempotent no-op`() {
        ...
        client.withTransaction { tx ->
            tx.tags.update(tag.id) { posts.add(post.id) }.save(testViewerContext).orRollback()
        }.getOrThrow()
        assertEquals(1L, junctionRowCount())
    ...
    fun `remove from the Tag side clears the junction row written from the Post side`() {
        ...
        client.withTransaction { tx ->
            tx.tags.update(tag.id) { posts.remove(post.id) }.save(testViewerContext).orRollback()
        }.getOrThrow()
    ...
    fun `set from the Tag side replaces the post set for that tag`() {
        ...
        val postA = client.posts.create { title = "A" }.saveAndLoad(testViewerContext).getOrThrow()
        val postB = client.posts.create { title = "B" }.saveAndLoad(testViewerContext).getOrThrow()
        val postC = client.posts.create { title = "C" }.saveAndLoad(testViewerContext).getOrThrow()

        // Seed [A, B] from the Tag side.
        client.withTransaction { tx ->
            tx.tags.update(tag.id) { posts.set(listOf(postA.id, postB.id)) }.save(testViewerContext).orRollback()
        }.getOrThrow()
        assertEquals(listOf(postA.id, postB.id).sorted(), linkedPostIds(tag.id))

        // Replace with [A, C]: adds C, removes B, keeps A — idempotently.
        client.withTransaction { tx ->
            tx.tags.update(tag.id) { posts.set(listOf(postA.id, postC.id)) }.save(testViewerContext).orRollback()
        }.getOrThrow()
