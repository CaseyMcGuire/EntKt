...
class FlywayMigrationWorkflow(
    ...
    private fun buildMigrationContent(
        ...
        if (result.manual.isNotEmpty() && manualMode == ManualMode.ACKNOWLEDGE_AND_ADVANCE) {
            ...
            sb.appendLine("-- Candidate DDL for each destructive operation is emitted COMMENTED OUT")
            sb.appendLine("-- under its checklist item — a STARTING POINT, not necessarily complete:")
            sb.appendLine("-- some need a data backfill first (e.g. SET NOT NULL) or a USING clause")
            sb.appendLine("-- (e.g. a type change), and a PRIMARY KEY change has no single statement")
            sb.appendLine("-- (its checklist note explains it, with no candidate DDL). Review and")
            sb.appendLine("-- complete each, then uncomment and place it in the correct position")
            sb.appendLine("-- relative to the auto-generated statements that follow the guard.")
            sb.appendLine("-- Ordering matters: a manual step may depend on an auto op (e.g. a")
            sb.appendLine("-- unique index over a newly added column must run AFTER that column),")
            sb.appendLine("-- and vice versa. Do not assume the SQL goes where this guard sits.")
            sb.appendLine("--")
