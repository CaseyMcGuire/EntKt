...
> time, takes the AND of the resulting `sourceSpec.predicates`,
> wraps it in `Predicate.HasEdgeWith` / `Predicate.HasM2MEdgeFrom`,
> and returns a `TraversalSourceResult` carrying both the bridge
> and the source-step annotations
> (see `read-path-interceptors.md` §"Source state is snapshotted at
> `queryX()` time" and `codegen/.../QueryGenerator.kt` around the
> `deferredSourceStep = { ... }` block). That deferral is what
> lets `*OrError` catch source-step rejections as
> `Err(QueryRejected)` and applies source tenant / soft-delete /
> max-limit interceptors. The code blocks in this section
> illustrate **only the typing changes**, not the deferred-
> execution control flow; the same typing applies inside the
> deferred lambda.
...
val parent: Predicate<User>? =
    sourceSpec.predicates.reduceOrNull { acc, p -> Predicate.And(acc, p) }
val bridge: Predicate<Post> =
    if (parent != null) Predicate.HasEdgeWith<Post, User>("author", parent)
    else Predicate.HasEdge<Post>("author")
```
...
val bridge: Predicate<Tag> =
    Predicate.HasM2MEdgeFrom<Tag, Post>(
        sourceTable = "posts",
        edgeName = "tags",
        sourceFilter = parent,  // Predicate<Post>?
    )
```
...
- the `deferredSourceStep` lambda generated by
  `QueryGenerator.kt` continues to call
  `sourceQ.runReadInterceptors(EDGE_TRAVERSAL, QUERY)` and build
  the bridge from `sourceSpec.predicates` — typed-RFC change
  does NOT regress to eager `combinedPredicate()` bridging
  (regression test for the deferred-execution contract)
