...
The original edge mutation RFC has been split into four smaller RFCs. They are
related, but they should be reviewed and implemented independently:

1. [To-One Assignment And Nullability](edge-mutation-to-one-assignment-nullability.md)
2. [Many-To-Many Schema Modeling](edge-mutation-m2m-schema-modeling.md)
3. [Link-Table M2M Mutation Helpers](edge-mutation-link-table-helpers.md)
...
Splitting the RFCs makes it possible to land the to-one behavior independently
while continuing to refine the more complex M2M and transaction design.
...
1. [To-One Assignment And Nullability](edge-mutation-to-one-assignment-nullability.md)
2. [Many-To-Many Schema Modeling](edge-mutation-m2m-schema-modeling.md)
3. [Transaction And Locking Semantics For Edge Mutations](edge-mutation-transaction-locking-semantics.md)
