...
- Mixed-mode rejection has two layers. The initial Phase 2 rule
  rejected replacement-vs-delta (`set(...)` and `add(...)` /
  `remove(...)` for the same edge in one mutation). The
  same-id mixed-direction rule (`add(x); remove(x)` or
  `remove(x); add(x)` for the same `x`) landed as a follow-up
  spec-conformance commit on top of the original V1 implementation
  to align with this RFC's "disjoint by construction" claim. Both
  rules fire at the offending mutator call site under normal usage,
  so callers see `IllegalStateException` immediately. The op-log
  fields (`_adds` / `_removes` / `_requestedSet`) are therefore
  disjoint by construction; downstream stages
  (`_buildPendingEdgeOps`, `_buildEdgeChanges` / runtime
  `computeEdgeChanges`) don't have to handle paired-cancellation
  state, and the `EdgeChanges` intent fields are also disjoint when
  validation / privacy observes them.
- The defense-in-depth mixed-mode check at save preflight is the
  backstop for state that bypassed the per-call mutator throw (e.g.
  reflection writing the op lists directly). It throws the same
  `IllegalStateException` shape, but only fires **after** the
  transaction and capability preflights — so a malformed save
  outside a transaction surfaces `TransactionRequiredException`
  first on that path.
...
The two sets are disjoint by construction: same-id mixed-direction calls are
rejected at the builder call site (see "Generated Builder Shape"), so no id
can appear in both. The shape aligns with
...
- `PendingEdgeOps` exposes the same intent field names as `EdgeChanges`
  (`requestedSet`, `requestedAdds`, `requestedRemoves`); `requestedAdds` and
  `requestedRemoves` are disjoint by construction (same-id mixed-direction
  rejected at the builder call site). `PendingEdgeOps` does not expose
  `added` / `removed` because junction state has not been read yet, and does
  not expose the raw ordered operation log
