...
class EntityRepositoryTest {
    ...
    @Test
    fun `both repository variants bind LOAD rules without resolving a client during construction`() {
        for (inTransaction in listOf(false, true)) {
            for (explicit in listOf(false, true)) {
                val fixture = Fixture(inTransaction)
                val contexts = mutableListOf<PrivacyRuleContext<RuleClient>>()
                fixture.loadPrivacyRules += PrivacyRule<RuleClient, Widget> { context, entity ->
                    contexts += context
                    if (entity.id == 1L) PrivacyDecision.Allow else PrivacyDecision.Deny("hidden")
                }

                val repo: WidgetReadSurface = if (explicit) fixture.explicit else fixture.generated

                assertEquals(0, fixture.clientResolutions)
                assertTrue(contexts.isEmpty())
                fixture.loadPrivacyRules.clear()
                val ruleClient = RuleClient("current-${fixture.driver.scope}")
                fixture.client = ruleClient
                val visible = Widget(1L, "visible")
                val hidden = Widget(2L, "hidden")

                val evaluation = repo.evaluateLoadPrivacy(viewerContext, listOf(visible, hidden))

                assertSame(visible, evaluation.allowedSubjects().single())
                val denial = evaluation.deniedOutcomes().single()
                assertSame(hidden, denial.subject)
                assertEquals("hidden", denial.reason)
                assertEquals(2, contexts.size)
                contexts.forEach { context ->
                    assertSame(viewerContext, context.viewerContext)
                    assertSame(ruleClient, context.client)
                }
                assertEquals(1, fixture.clientResolutions)
            }
        }
    }
    ...
    fun `base query executor uses the scoped read host and its execution guard`() {
        for (inTransaction in listOf(false, true)) {
            for (explicit in listOf(false, true)) {
                ...
                assertTrue(fixture.readViewers.isEmpty())
                val failure = IllegalStateException("read scope ended")
    ...
    private class Fixture(inTransaction: Boolean = false) : MutationRuntime, ReadQueryExecutionHost {
        ...
        val loadPrivacyRules = mutableListOf<BatchPrivacyRule<RuleClient, Widget>>()
