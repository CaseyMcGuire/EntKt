...
class EntGenerator(
    ...
    fun generate(schemas: List<SchemaInput>): List<FileSpec> {
        ...
        // Different files can still emit classifiers with the same package
        // name. For example, schema `Post` emits the top-level alias
        // `PostLoadBatchPrivacyRule`, which would collide with an entity whose
        // schema name is itself `PostLoadBatchPrivacyRule`. File-name checks
        // cannot see declarations inside PostPrivacy.kt, so inspect the actual
        // generated top-level types and aliases before returning them.
        val topLevelTypes = files.flatMap { file ->
            file.members.mapNotNull { member ->
                when (member) {
                    is TypeSpec -> member.name?.let { Triple(it, file.name, "type") }
                    is TypeAliasSpec -> Triple(member.name, file.name, "typealias")
                    else -> null
                }
            }
        }
        val topLevelTypeCollisions = topLevelTypes
            .groupBy { it.first }
            .filterValues { it.size > 1 }
        if (topLevelTypeCollisions.isNotEmpty()) {
            error(
                "Generated top-level type collisions detected:\n" +
                    topLevelTypeCollisions.entries.joinToString("\n") { (name, declarations) ->
                        "  - '$name' is generated by " + declarations.joinToString { (_, file, kind) ->
                            "$kind in $file.kt"
                        }
                    },
            )
