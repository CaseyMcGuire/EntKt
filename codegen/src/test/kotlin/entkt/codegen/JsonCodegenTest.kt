...
class JsonCodegenTest {
    ...
    fun `create preparation detaches typed JSON before the write map and candidate`() {
        val create = gen().getValue("JsonArticleCreateConverter")
        assertTrue(
    ...
    fun `rules share JSON while update preparation still detaches requested patches`() {
        ...
        val createBinding = repo.substringAfter("createOperations = buildCreateOperations(")
            .substringBefore("updateOperation = buildUpdateOperation(")
        ...
        val updateBinding = repo.substringAfter("updateOperation = buildUpdateOperation(")
            .substringBefore("deleteOperation = buildDeleteMutationOperation(")
    ...
    fun `generic create preparation detaches the typed value without a cast`() {
        val create = genGeneric().getValue("JsonBoardCreateConverter")
        assertTrue(
